...
set GOOS=wasip1
set GOARCH=wasm
go build -o sqlc-gen-python-arche.wasm plugin/main.go

REM ──────────────────────────────
REM 3) CALCULATE SHA‑256
REM ──────────────────────────────
for /f %%i in ('certutil -hashfile sqlc-gen-python-arche.wasm SHA256 ^| findstr /v "hash"') do set "SHA256_HASH=%%i"
...
for %%D in (%TARGET_DIRS%) do (
    ...
    REM Copy the plugin
    xcopy /Y /Q "sqlc-gen-python-arche.wasm" "%%D\"
