...
class WidgetConfig(pydantic.BaseModel):
    """Nested config shape with edge-case field types.

    ``extra="forbid"`` makes an unexpected key in the stored jsonb fail
...
class TextPayload(pydantic.BaseModel):
    """Union member A: a text payload (``kind="text"``)."""
...
class NumberPayload(pydantic.BaseModel):
    """Union member B: a numeric payload (``kind="number"``)."""
